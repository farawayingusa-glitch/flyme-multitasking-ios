...
strings "$keyboard" | grep -q "keyboard-app-ctor-v53"
strings "$keyboard" | grep -q "keyboard-app-ready-v53"
strings "$keyboard" | grep -q "keyboard-dismiss-request-reset-v1"
strings "$runtime" | grep -q "dock-input-block-v1"
strings "$runtime" | grep -q "dock-input-block publish"
strings "$runtime" | grep -q "schema=19"
strings "$runtime" | grep -q "Stable build 0.9.56"
strings "$keyboard" | grep -q "dock-input-block-v1"
