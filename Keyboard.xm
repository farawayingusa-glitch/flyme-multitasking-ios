...
// This module is deliberately a narrow UIKit geometry adapter. SpringBoard
// owns the keyboard host, window, touch routing, and session lifetime. UIKit
// clients only receive the physical keyboard reference size and the overlap
// for the currently hosted application Scene.
...
static BOOL FLMSceneMatchesKeyboardRoute(UIWindowScene *scene) {
    ...
    // A third-party keyboard extension owns its own remote keyboard Scene. The
    // active generation already scopes it to the selected application.
    if (FLMKeyboardExtensionProcess) {
        return YES;
    }
    if (!FLMKeyboardTargetApplication) {
    ...
    // SpringBoard's private scene identifier and UISceneSession's persistent
    // identifier are not always identical on iOS 16. The bundle hash already
    // limits this adapter to one application process, so a single connected
    // UIWindowScene is an unambiguous fallback.
...
static void FLMReloadKeyboardRoute(void) {
    ...
    if (previousTargetApplication && previousGeneration != 0 &&
        ...
        // Only a centered-card generation change ends the app responder. A
        // native keyboard hide inside the same generation must never close the
        // card or be turned into a second synthetic dismissal.
...
%ctor {
    @autoreleasepool {
        ...
        // Hook installation is side-effect free. UIScreen and UIApplication are
        // only queried later, after an actual routed method call.
        FLMInstallRemoteKeyboardGeometryIfAvailable();
        FLMReloadKeyboardRoute();
        if (FLMKeyboardRouteActive) {
            FLMPublishDiagnosticEvent(
