# Flyme Multitasking 0.9.66 - physical corner repair
...
0.9.66 使用固定屏幕坐标连接窗口触摸与显示画布，修复窗口/根视图方向不同造成的触发位置偏移；热点窗口保持可用并在触摸入口实时判断方向，避免漏发旋转通知后仍走旧入口。保护手势不再拦截轮盘展开手势。横屏键盘使用实际可见区域，不套用竖屏高度缓存和 56pt 额外保护区；卡片几何发布使用缩放后的实际尺寸。

已通过双架构语法检查和几何/键盘逻辑回归；实际 iOS 私有窗口行为仍需安装后复测。
