...
当前交付为 `Stable build 0.9.59`：以已稳定的 0.9.58 发热优化版为基础，继续减少插件
闲置时的 CPU 唤醒；触摸 display link 在没有待处理帧时立即释放，轮盘未固定且小窗隐藏时
停止锁监控定时器。多任务、Dock、键盘 Scene、触摸所有权、动画时长和最高刷新率策略保持不变。
