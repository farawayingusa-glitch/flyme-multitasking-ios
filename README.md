...
当前交付为 `Stable build 0.9.55`：源码从确认稳定的 `0.9.41`
竖屏提交 `8c1404f` 重新分支，只增加关闭小窗前的应用侧第一响应者清理、
微信主进程显式适配入口、Dock 跟手链路的设备最高刷新率请求、挂靠阈值后的
即时内容冻结、常驻穿透触摸闸门、卡片窗口自身的首触摸内容阻断，
以及与卡片进度联动的小白条渐隐/渐显；未合入
0.9.44 之后的遮罩、键盘等待状态机或自适应渲染架构。
