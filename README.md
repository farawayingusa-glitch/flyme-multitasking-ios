...
当前交付为 `Stable build 0.9.58`：以已稳定的 0.9.57 为基础，只优化耗电与发热热路径；
Release 默认关闭诊断链路，合并共享键盘状态写盘，去重 Darwin 通知，并缓存锁屏检测与
圆角偏好读取。多任务、Dock、键盘 Scene、触摸所有权、动画时长和最高刷新率策略保持不变。
